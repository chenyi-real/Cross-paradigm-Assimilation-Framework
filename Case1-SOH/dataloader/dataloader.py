...
class DF():
    ...
    def read_one_csv(self,file_name,nominal_capacity=None):
        ...
        if nominal_capacity is not None:
            ...
            if nominal_capacity != 1.0:
                df['capacity'] = df['capacity']/nominal_capacity
                
            # SOH normalization 这句话会导致结果变差，怀疑是因为min-max归一化导致的
            # df.iloc[:,-1] = 2*(df.iloc[:,-1] - df.iloc[:,-1].min())/(df.iloc[:,-1].max() - df.iloc[:,-1].min()) - 1
            ...
            df["cycle index"] = df["cycle index"].astype(float)
            df.iloc[:,:-1] = f_df
    ...
    def load_one_battery(self, path, nominal_capacity=None):
        """
        Read a csv file and divide the data into x and y.
        If the CSV contains a 'Cycle' column, it will be removed.
        """
        df = self.read_one_csv(path, nominal_capacity)
    ...
    def load_all_battery(self,path_list,nominal_capacity):
        ...
        for path in path_list:
            (x1, y1), (x2, y2), (x3, y3) = self.load_one_battery(path, nominal_capacity)
            # print(path)
        ...
        # Condition 4
        test_loader_4 = DataLoader(TensorDataset(tensor_X3, tensor_Y3),
                                 batch_size=self.args.batch_size,
                                 shuffle=False)
...
class MITdata(DF):
    def __init__(self,root='../data/MIT data',args=None, normalization=True):
        super(MITdata, self).__init__(args)
        self.root = root
        self.batchs = ['2017-05-12','2017-06-30']
...
class NASAdata(DF):
    def __init__(self,root='../data/MIT data',args=None, normalization=True):
        ...
        self.root = root
        self.batchs =  ['B0005','B0006', 'B0007', 'B0018']
        self.normalization = normalization
        if self.normalization:
            self.nominal_capacity = 1.0
        else:
