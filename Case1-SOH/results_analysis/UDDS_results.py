...
def main(root_item, target_dataset='UDDS', test_id=None, dir_name='dir_name'):
    print('Now!!!!!', root_item, target_dataset, test_id)
    if test_id is None:
        root = f'./{dir_name}/{target_dataset}-{root_item}/'
    else:
        root = f'./{dir_name}/{target_dataset}-{root_item}/{test_id}'
    writer = pd.ExcelWriter(f'./{dir_name}/{root_item}-{target_dataset}-results.xlsx')
...
if __name__ == '__main__':
   ...
   main('Ours', 'UDDS', dir_name='exp_20251128_distill')
