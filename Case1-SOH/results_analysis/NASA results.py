...
class Results:
    ...
    def parser_label(self):
        ...
        split_point = np.where(diff>0.4)[0]
    ...
    def get_test_results(self,e):
        ...
        results_dict['channel'] = list(range(len(results_dict['MAE'])))
        return results_dict
    ...
    def get_experiment_average(self):
        ...
        for i in range(1, 11):
            ...
            df_value_list.append(df[['MAE', 'MAPE', 'RMSE']].values[:8])
        channel = df['channel']
...
def main(root_item, dataset, model, test_id, root='./reuslts'):
    results_root = f'{root}/results of {root_item}/{dataset} results-{test_id}/'
    writer = pd.ExcelWriter(f'{root}/{model}-{dataset}-results-{test_id}.xlsx')
    results = Results(results_root)
...
if __name__ == '__main__':
   ROOT = '/workspace/code/nc_proj/soh_pinn_v2/PINN4SOH-main/new_results_v7_table4/'
   ROOT = '/workspace/code/nc_proj/soh_pinn_v2/PINN4SOH-main/new_results_10/'
   main('PINNdebug', 'NASA', 'Ours', 'B0005', root = ROOT)
#    main('PINNdebug', 'NASA', 'Ours', 'B0006', root = ROOT)
#    main('PINNdebug', 'NASA', 'Ours', 'B0007', root = ROOT)
#    main('PINNdebug', 'NASA', 'Ours', 'B0018', root = ROOT)

#    main('PINNv2', 'NASA', 'PINN', 'B0005', root = ROOT)
#    main('PINNv2', 'NASA', 'PINN', 'B0006', root = ROOT)
#    main('PINNv2', 'NASA', 'PINN', 'B0007', root = ROOT)
#    main('PINNv2', 'NASA', 'PINN', 'B0018', root = ROOT)
