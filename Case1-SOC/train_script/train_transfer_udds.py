...
from configs.config_transfer_udds import (
    ...
    EPOCHS, LR, PATIENCE, MODELS_DIR,PRETRAIN_WEIGHTS, METHOD, EXP_DIR
)
...
def train_tcn_on_table(train_df, val_df, test_df, device=DEVICE, run_id=1, fname=None):
    """
    使用 TwoBranchFramework 模型进行训练，
    加载 PRETRAIN_WEIGHTS 的预训练参数，并冻结除 self.fc 以外的层。
    """
    # 构建 DataLoader
    ...
    if PRETRAIN_WEIGHTS is not None and os.path.exists(PRETRAIN_WEIGHTS):
        print(f"🔹 加载预训练模型参数：{PRETRAIN_WEIGHTS}")
        pretrained_state = torch.load(PRETRAIN_WEIGHTS, map_location=device)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if va_loss < best_val - 1e-12:
            ...
            print(f"[TCN] ep {ep:02d} Test:  RMSE={rmse:.6f}  MAE={mae:.6f}")
        else:
            bad += 1
            if bad >= PATIENCE:
    ...
    if best_state is not None:
        ...
        torch.save(model.state_dict(), os.path.join(root, 'best_model_fc_finetuned.pth'))
        print(f"💾 已保存微调后模型: {os.path.join(root, 'best_model_fc_finetuned.pth')}")
