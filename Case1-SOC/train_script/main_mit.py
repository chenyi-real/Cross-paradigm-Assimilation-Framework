...
import inspect
import configs.config_mit as config_mit
from configs.config_mit import DATA_DIR, CSV_FILES, DEVICE, RESULTS_XLSX, PLOTS_DIR, PLOT
...
def main():
    ...
    os.makedirs(config_mit.EXP_DIR, exist_ok=True)
    cfg_path = os.path.join(config_mit.EXP_DIR, "config.yaml")

    with open(cfg_path, "w", encoding="utf-8") as f:
        for k, v in vars(config_mit).items():
            # 跳过内置变量和模块等
