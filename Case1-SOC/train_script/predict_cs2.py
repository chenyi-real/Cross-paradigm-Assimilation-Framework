...
from dataloader import build_dataloaders_from_cs2_df, load_all_csv_in_folder
from train import _epoch, seed_everything

from configs.config_cs2 import (
    DEVICE, WINDOW, STRIDE, BATCH_SIZE, NUM_WORKERS, MODELS_DIR
)
from configs.config_cs2 import DATA_DIR, CSV_FILES

RESULTS_DIR = './test_results_20250311/baseline/CS2/'


def extract_lte_dataset_to_dataframe(lte):
    """
    将 DataLoader 中的 dataset 提取出来，保存成 DataFrame。
    这里尽量做成通用写法，适配 TensorDataset / 自定义 Dataset。

    返回：
        df_windows: 每一行对应一个窗口样本
    """
    dataset = lte.dataset
    rows = []
...
def build_row_level_pred_df(df_raw, y_true, y_pred, window, stride):
    ...
    pred_idx = 0  # 当前使用到的预测序号

    for cycle_id, df_cycle in df_out.groupby(["Date", "cycle"]):
...
def test_one_csv(fname: str, run_id: int = 1):
    ...
    _, _, lte, df_te = build_dataloaders_from_cs2_df(
...
def main():
    ...
    for fname in CSV_FILES:
        ...
        print(f"  window_pred.csv")
        print(f"  lte_windows_with_pred.csv")
