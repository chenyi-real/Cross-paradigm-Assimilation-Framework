...
from dataloader import build_dataloaders_from_mit_df
...
def train_tcn_on_table(df, device=DEVICE, run_id=1, fname=None):
    ltr, lva, lte = build_dataloaders_from_mit_df(df, WINDOW, STRIDE, BATCH_SIZE, NUM_WORKERS)
    ...
    # Ours
    model = TwoBranchFramework().to(device)  # Ours
