...
if __name__=='__main__':
    ...
    save_path = './work_dir_table9/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)


    root = './work_dir_train'
    SRC_DATA = ['DST', 'US06', 'FUDS']
    TRG_DATA = {'DST': 'FUDS',
                'US06':'FUDS',
                'FUDS': 'US06'
                }
    ...
    for data in SRC_DATA:
        for temp in TEMP:
            for level in LEVEL:
                print('-'*100)
                print('Now is in: ', f'{data}-{temp}C-{level}SOC')
                exp_results = []
                for i in tqdm(range(10)):
                    # 加载目标模型
                    checkpoint = torch.load(f'{root}/{TRG_DATA[data]}/{TRG_DATA[data]}-{temp}C-{level}SOC/model_{i}.pt')
