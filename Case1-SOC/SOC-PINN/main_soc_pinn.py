...
class StrideDataset(Dataset):
    ...
    def __init__(self, df, enc_seq_len, target_seq_len, stride=5):
        ...
        num_samples = (self.length - enc_seq_len) // stride 
...
class CustomDataset(Dataset):
    def __init__(self, df):
        ...
        self.x = df.iloc[:-1, 6:10].values
        #self.x = np.reshape(x, (x.shape[0], 1, x.shape[1]))
        self.y = df.iloc[:-1, 7:8].values
        
        self.y_soc = df.iloc[:-1, 17:18].values
...
def main(args, data='USO6', temp='0', level='80', root='./work_dir/'):
    pass
    print(1)
    
    # args
    lr = 0.003   # 0.00003
    epoch = args.epochs   # 800
    ...
    train_dataset_2 = StrideDataset(train_df, 50, 20, stride=5)
    # test_dataset_2 = StrideDataset(test_df, 50, 20, stride=5)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.warmup_lr)
    ...
    for i in progress:
        ...
        for (inputs_soc, inputs_c, outputs_v, dec_inputs_v, outputs_soc) in train_dataloader:
            optimizer.zero_grad()
            ...
            result, pred_soc = model(inputs_soc.float().to(device), outputs_soc.float().to(device), inputs_c.float().to(device), dec_inputs_v.float().to(device), src_mask_soc, src_mask_c, tgt_v_mask)
            ...
            loss_v = criterion(result, outputs_v.float().to(device))
            loss_soc = criterion(pred_soc, outputs_soc.float().to(device))  
            ######一个训练小技巧                                                                          
            # if i % 4 == 0 or i % 8 == 0:
            #     loss = 0.4 * loss_v + 0.6 * loss_soc
            # else:
            #     loss = loss_v
            loss = loss_v
            loss.backward()
            ...
            batchloss += loss
            batchloss_v += loss_v
            batchloss_soc += loss_soc

        current_lr = scheduler.step()
        loss_arr.append((batchloss.cpu().item() / len(train_dataloader)))
        progress.set_description("current_lr: {:0.6f}, loss: {:0.6f}, loss_v: {:0.6f}, loss_soc: {:0.6f}".format(
                                                current_lr,
                                                batchloss.cpu().item() / len(train_dataloader),
                                                batchloss_v.cpu().item() / len(train_dataloader),
                                                batchloss_soc.cpu().item() / len(train_dataloader)
                                                ))
        
        if (batchloss.cpu().item() / len(train_dataloader)) < best_loss:
            best_loss = (batchloss.cpu().item() / len(train_dataloader))
            best_model = model.state_dict()
            torch.save(best_model, os.path.join(workdir, f'best_model.pt'))

        # if i > epoch-50:
        #     torch.save(model.state_dict(), os.path.join(workdir, f'model_{model_save_id % 10}.pt'))
        #     model_save_id +=1
    ...
    plt.savefig(os.path.join(workdir, "loss.png"))
    # print(loss_arr[1500])
    # print(loss_arr[2400])

    # 保存模型
    # torch.save(model, workdir + 'model.pt')

    test(workdir=workdir, dataset_name=dataset_name, checkpoint=best_model)
...
def test(workdir, dataset_name='FUDS-0C-80SOC', checkpoint=None):
    ...
    predictions_zerosum = torch.zeros(49)
    ...
    plt.plot(dataset_d.y[49:], color='red', alpha=0.7)
    plt.plot(predictions_v[49:], color='blue', linewidth=0.7)
    ...
    df = pd.DataFrame(predictions_v[49:])
    df.to_csv(os.path.join(workdir, '2_F_fuds_v.csv'))

    # 画SOC的图
    plt.figure(figsize=(10,4))
    plt.plot(dataset_d.y_soc[49:], color='red', alpha=0.7)
    plt.plot(predictions_soc[49:], color='blue', linewidth=0.7)
    plt.title('Actual vs Forecast')
    plt.legend(['Actual', 'Forecast'])
    plt.xlabel('Time Steps')
    # plt.xlim([2800,3000])
    ...
    df = pd.DataFrame(predictions_soc[49:])
    ...
    print("v-mae : ", mean_absolute_error(dataset_d.y[49:10000]/ 4.2, predictions_v[49:10000] /4.2))
    print("v-mse : ", mean_squared_error(dataset_d.y[49:10000]/4.2, predictions_v[49:10000]/4.2))
    print("v-rmse : ", np.sqrt(mean_squared_error(dataset_d.y[49:10000]/4.2, predictions_v[49:10000]/4.2)))

    print("soc-mae : ", mean_absolute_error(dataset_d.y_soc[49:10000]/100.0, predictions_soc[49:10000]/100.0))
    print("soc-mse : ", mean_squared_error(dataset_d.y_soc[49:10000]/100.0, predictions_soc[49:10000]/100.0))
    print("soc-rmse : ", np.sqrt(mean_squared_error(dataset_d.y_soc[49:10000]/100.0, predictions_soc[49:10000]/100.0)))
...
if __name__=='__main__':
    ...
    if index == 1:
        ...
        level_id = 0
        for temp in Temp:
            main(args, data=Data[data_id], temp=temp, level=Level[level_id], root=root)
    elif index == 2:
        data_id = 1
        level_id = 0
        for temp in Temp:
